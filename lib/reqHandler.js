...
function serverReadHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn._dumpObj(pathObj.oid, pathObj.iid, readCallback);
    } else if (target.type === TTYPE.rsc) {
        cn.readResrc(pathObj.oid, pathObj.iid, pathObj.rid, readCallback);
    }
}
...
function serverWriteHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn.writeResrc(pathObj.oid, pathObj.iid, pathObj.rid, value, writeCallback);
    }
}
