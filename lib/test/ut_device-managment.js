...
describe('coap-node device-managment test', function() {
    ...
    describe('coap-shepherd tries to discover', function() {
        ...
        it('discover - instence', function (done) {
            var result = { 
                    attrs: { pmin: 1, pmax: 60 },
                    resrcList: { 0: ['5700', '5701', '5702', '5703', '5704'] } 
                };
    ...
    describe('coap-shepherd tries to writeAttr', function() {
        it('writeAttr - resource', function (done) {
            var attrs = {
                pmin: 10,
                pmax: 90,
                gt: 0,
            ...
            remoteNode.writeAttr('/temperature/0/sensorValue', attrs, function (err, msg) {
                if (msg.status === '2.04') done();
            });
        });

        it('writeAttr - instence', function (done) {  
            var attrs = {
                pmin: 10,
                pmax: 90,
                gt: 0,
            ...
            remoteNode.writeAttr('/temperature/0', attrs, function (err, msg) {
                if (msg.status === '2.04') done();
            });
        });

        it('writeAttr - object', function (done) {
            var attrs = {
                pmin: 10,
                pmax: 90,
                gt: 0,
            ...
            remoteNode.writeAttr('/temperature', attrs, function (err, msg) {
                if (msg.status === '2.04') done();
            });
        });

        it('writeAttr - bad path', function (done) {
            var attrs = {
                pmin: 10,
                pmax: 90,
                gt: 0,
            ...
            remoteNode.writeAttr(3303, attrs, function (err, msg) {
                if (err) done();
            });
        });

        it('writeAttr - bad attrs', function (done) {
            remoteNode.writeAttr('/temperature/0/sensorValue', 'x', function (err, msg) {
                if (err) done();
            });
        });

        it('writeAttr - bad attrs', function (done) {          
            remoteNode.writeAttr('/temperature/0/sensorValue', [ 10 ], function (err, msg) {
                if (err) done();
            });
        });
