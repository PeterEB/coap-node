...
var defaultPort = config.defaultPort || 5684,
...
function CoapNode (clientName, devAttrs) {
    ...
    this.lifetime = Math.floor(devAttrs.lifetime) || config.lifetime;
    this.version = devAttrs.version || config.version;

    this.ip = devAttrs.ip || 'unknown';
    this.mac = devAttrs.mac || 'unknown';
    this.port = devAttrs.port || defaultPort;
...
CoapNode.prototype.start = function (deviceInfo, callback) {
    var self = this;

    deviceInfo = deviceInfo || {};

    if (_.isFunction(deviceInfo)) {
        callback = deviceInfo;
        deviceInfo = {};
    }
    ...
    this.so.device = {
        0: {       // oid = 3
            manuf: deviceInfo.manuf || 'sivann',        // rid = 0
            model: deviceInfo.model || 'cnode-01',      // rid = 1
            serial: deviceInfo.serial || 'c-0000',      // rid = 2
            firmware: deviceInfo.firmware || 'v1.0',    // rid = 3
            devType: deviceInfo.devType || 'generic',   // rid = 17
            hwVer: deviceInfo.hwVer || 'v1.0',          // rid = 18
            swVer: deviceInfo.swVer || 'v1.0',          // rid = 19
            availPwrSrc: deviceInfo.availPwrSrc || 'line',
            pwrSrcVoltage: deviceInfo.pwrSrcVoltage || '3.3V'
        }
    };
    ...
    this._updateNetInfo(function (err, msg) {
        if (err) 
            throw err;
        else {
            self.emit('ready');
            if (_.isFunction(callback)) callback(null, msg);
        }
    });
};
...
CoapNode.prototype.readResrc = function (oid, iid, rid, callback) {
    ...
    function invokeCb(err, val, cb) {
        ...
        if (!_.isNil(val)) _checkAndReportResrc(self, target.oidKey, iid, target.ridKey, val);
    }
...
CoapNode.prototype.writeResrc = function (oid, iid, rid, value, callback) {
    ...
    function invokeCb(err, val, cb) {
        ...
        if (!_.isNil(val)) _checkAndReportResrc(self, oid, iid, rid, val);
    }
...
CoapNode.prototype.register = function (ip, port, callback) {
    ...
    var self = this,
        reqObj = { 
            ...
            port: port, 
            pathname: '/rd',
            query: 'ep=' + self.clientName + '&lt=' + self.lifetime + '&lwm2m=' + self.version + '&mac=' + self.mac,
...
CoapNode.prototype._getAttrs = function (oid, iid, rid) {
    ...
    if (arguLen === 3 && rid === undefined)
        arguLen = 2;
    else if (arguLen === 2 && iid === undefined)
        arguLen = 1;
...
function _serverExecuteHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn.execResrc(pathObj.oid, pathObj.iid, pathObj.rid, argus, function (status, data) {
            if (status) {
                if (status === ERR.unexecutable) 
                    rsp.code = RSP.notallowed;
                else 
                    rsp.code = status;
...
function _checkAndReportResrc(cn, oidKey, iid, ridKey, val) {
    var target = cn._target(oidKey, iid, ridKey),
    ...
    if (rAttrs.cancel && iAttrs.cancel)
        return false;
    ...
    if (rAttrs.mute && !rAttrs.cancel) {
        setTimeout(function () {
            _checkAndReportResrc(cn, oidKey, iid, ridKey, val);
        }, rAttrs.pmin * 1000);
    } else if (!rAttrs.mute && chkRp && !rAttrs.cancel && _.isFunction(rpt.write)) {
        rpt.write(val);
    }

    if (iAttrs.mute && !iAttrs.cancel) {
        setTimeout(function () {
            _checkAndReportResrc(cn, oidKey, iid, ridKey, val);
        }, iAttrs.pmin * 1000);
    } else if (!iAttrs.mute && chkRp && !iAttrs.cancel && _.isFunction(iRpt.write)) {
        iObj[ridKey] = val;
        iRpt.write(iObj);
    }
    ...
    if (enable) {
        cn._socketServerChker = setInterval(function () {
            _.forEach(cn.servers, function (server, key) {
                ...
                if (using === false && server._port !== defaultPort && server._port !== cn.port) {
                    server.close();
                    cn.servers[key] = null;
...
function _dumpObjFilter(dumpObj) {
    var excludedKeys = [ '_unreadable_', '_exec_'],
        dumped = {};

    _.forEach(dumpObj, function (n , key) {
        if (!_.includes(excludedKeys, n)) {
            if (_.isObject(n))
