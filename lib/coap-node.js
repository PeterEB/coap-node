...
function _serverCancelObserveHandler (node, req, rsp) {
    var pathObj = cutils.getSoKeyObj(req.url),
        target = node._target(pathObj.oid, pathObj.iid, pathObj.rid);
    ...
    if (!target.exist) {
        ...
        node._disableReport(pathObj.oid, pathObj.iid, pathObj.rid, function (err, val) {
            rsp.code = RSP.content;
            rsp.end();
        });
    }
...
function _heartbeat(cn, enable) {
    clearInterval(cn._pacemaker);
    cn._pacemaker = null;
