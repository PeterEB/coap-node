        ...
        _.forEach(value, function (rsc, rid) {
            self.so.write(oid, iid, rid, rsc, function (err, data) {
                ...
                if (err) {          // [TODO] reply to the original data?
                    chkErr = chkErr || err;
                    dump = data;    // If has a error, return error data. 
                    count = 0;
                } else {
    ...
    self.request(reqObj, agent, function (err, rsp) {
        if (err) {
            ...
            if (rsp.code === RSP.changed) {
                self._bssIp = rsp.rsinfo.address;
                self._bssPort = rsp.rsinfo.port;
...
CoapNode.prototype.configure = function (ip, port, opts) {   // opts: { lifetime pmax pmin }
    ...
    data.lwm2mSecurity = this.so.init('lwm2mSecurity', securityIid, {
    ...
    data.lwm2mServer = this.so.init('lwm2mServer', serverIid, {
        shortServerId: shortServerId,
...
CoapNode.prototype.register = function (callback) {
    var self = this,
        requestCount = 0,
    ...
    // [TODO] With Error
    if (_.isString(arguments[0]) && (_.isString(arguments[1]) || _.isNumber(arguments[1]))) {
        return this._register(arguments[0], arguments[1], this._idCounter('shortServerId'), function (err, rsp) {
            if (err) 
                chkErr = chkErr || err;
            // [TODO]
        });
    ...
    _.forEach(serverObjs, function (serverObj, iid) {
        ...
        _.forEach(securityObjs, function (securityObj, iid) {
            if (serverObj.shortServerId === securityObj.shortServerId)
                lwm2mServerURI = securityObj.lwm2mServerURI;
        });

        if (lwm2mServerURI) {
            serverInfo = getServerUriInfo(lwm2mServerURI);
            self._register(serverInfo.address, serverInfo.port, serverObj.shortServerId, function (err, rsp) {
                if (err) 
                    chkErr = chkErr || err;
                // [TODO]
            });
    ...
    if (chkErr)
        ...
        invokeCbNextTick(null, null, callback);     // [TODO]
};

/*******************************************************************/
/********  Deprecated: Use configure and register instead.  ********/
/*******************************************************************/
...
CoapNode.prototype.update = function (attrs, callback) {
    ...
    if (localStatus) {
        ...
        _.forEach(this.serversInfo, function (serverInfo, ssid) {
            this._update(serverInfo, updateObj, function (err, rsp) {
                // [TODO]
            });
        });
    }
...
CoapNode.prototype._disableAllReport = function (ssid) {
    ...
    if (_.isNil(ssid)) {
        ...
        disableReport(this, this.serversInfo[ssid]);
    }
};
