...
CoapNode.prototype._init = function () {
    ...
    this._startListener(function (err) {
        if (err) throw err;
    });
...
CoapNode.prototype.setDevAttrs = function (attrs, callback) {
    ...
    objListInPlain = _checkAndBuildObjList(self);

    if (objListInPlain !== false)
    ...
    if (_.isEmpty(updateObj)) {
        ...
        this.update(updateObj, callback);
    }
};
...
CoapNode.prototype.writeInst = function (oid, iid, value, callback) {
    var self = this,
        target = this._target(oid, iid),
        rsc = target.value,
        okey = cutils.oidKey(oid),
    ...
    if (!target.exist){
        callback(new Error('not found target.'), null);
    } else {
...
CoapNode.prototype.register = function (ip, port, callback) {
    ...
    var self = this,
        reqObj = { 
            ...
            payload: _checkAndBuildObjList(this),
            method: 'POST'
        },
        msg;
        ...
        this.request(reqObj, function (err, rsp) {
            if (err) {
                ...
                if (rsp.code === RSP.deleted) {
                    _.forEach(self.servers, function (server, idx) {
                        if (idx !== 0)
                            server.close();
                    });
...
CoapNode.prototype._startListener = function (callback) {
    ...
    server.on('request', function (req, rsp) {
        ...
        if (req.rsinfo.address !== self._serverIp) {
            rsp.code = RSP.badreq;
            rsp.end();
        } else {
...
CoapNode.prototype.request = function (reqObj, callback) {
    ...
    var self = this,
        agent = new coap.Agent({ type: ipProtocol }),
...
CoapNode.prototype._enableReport = function (oid, iid, rid, rsp, callback) {
    ...
    this._reporters[key] = { min: null, max: null, write: null, stream: rsp, port: this.servers[this.servers.length - 1]._port };
...
function _serverWriteHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn.writeInst(pathObj.oid, pathObj.iid, value, function (err, data) {
            if (err) {
                rsp.code = RSP.badreq;
                rsp.end();
            } else {
...
function _checkAndBuildObjList(cn) {
    var objListInPlain = '',
        newObjList = {};
    ...
    if (!_.isEmpty(cn.objList) && _.isEqual(cn.objList, newObjList))
...
function _heartbeat(cn, enable, rsp) {
    ...
    if (enable) {
        ...
        cn._hbStream.port = cn.servers[cn.servers.length - 1]._port;
...
function _checkAndCloseServer(cn, enable) {
    ...
    if (enable) {
        cn._serverChker = setInterval(function () {
            _.forEach(cn.servers, function (server, idx) {
                var using = false;
                ...
                if (using === false && server._port !== clientDefaultPort && server._port !== cn._hbStream.port) {
                    server.close();
                    cn.servers.splice(idx, 1);
                }
            });
        }, serverChkTime * 1000);  
    }
