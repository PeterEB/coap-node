...
CoapNode.prototype.readResrc = function (oid, iid, rid, callback) {
    ...
    this.so.read(oid, iid, rid, function (err, val) {
...
CoapNode.prototype._dumpObj = function (oid, iid, callback) {
    ...
    if (!target.exist) {
        ...
        this.so.dump(oid, callback);
    } else if (target.type === TTYPE.inst) {
        this.so.dump(oid, iid, callback);
    }
};
...
CoapNode.prototype.writeResrc = function (oid, iid, rid, value, callback) {
    ...
    this.so.write(oid, iid, rid, value, function (err, val) {
        if (!_.isNil(val)) helper.checkAndReportResrc(self, oid, iid, rid, val);
        callback(err, val);
    });
};
...
CoapNode.prototype._setAttrs = function (oid, iid, rid, attrs) {
    ...
    attrs.pmin = _.isNumber(attrs.pmin) ? attrs.pmin : this.so.get('lwm2mServer', 0, 'defaultMinPeriod');
    attrs.pmax = _.isNumber(attrs.pmax) ? attrs.pmax : this.so.get('lwm2mServer', 0, 'defaultMaxPeriod');
...
function init(cn, devResrcs) {
    ...
    cn.so.init('lwm2mServer', 0, {                  // oid = 1
    ...
    cn.so.init('connMonitor', 0, {      // oid = 4
        ip: cn.ip,                      // rid = 4
