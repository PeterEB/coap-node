...
function serverReadHandler (cn, req, rsp) {
    ...
    function readCallback(err, data) {
        if (err) {
            ...
            if (_.isPlainObject(data)) {
                ...
                rsp.setOption('Content-Format', 'text/plain');
    ...
    if (!target.exist) {
        ...
        cn.so.dump(pathObj.oid, readCallback, { restrict: true });
    } else if (target.type === TTYPE.inst) {
        cn.so.dump(pathObj.oid, pathObj.iid, readCallback, { restrict: true });
    } else if (target.type === TTYPE.rsc) {
        cn.so.read(pathObj.oid, pathObj.iid, pathObj.rid, readCallback, { restrict: true });
    }
}
...
function serverWriteHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn.so.write(pathObj.oid, pathObj.iid, pathObj.rid, value, writeCallback, { restrict: true });
    }
}
