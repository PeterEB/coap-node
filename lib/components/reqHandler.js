...
function serverReadHandler (cn, req, rsp) {
    ...
    function readCallback(err, data) {
        if (err) {
            if (data ===  TAG.unreadable || data === TAG.exec) {
    ...
    if (!target.exist) {
        ...
        cn.so.dump(pathObj.oid, readCallback);
    } else if (target.type === TTYPE.inst) {
        cn.so.dump(pathObj.oid, pathObj.iid, readCallback);
    } else if (target.type === TTYPE.rsc) {
        cn.so.read(pathObj.oid, pathObj.iid, pathObj.rid, readCallback);
    }
}
...
function serverWriteHandler (cn, req, rsp) {
    ...
    if (!target.exist) {
        ...
        cn.so.write(pathObj.oid, pathObj.iid, pathObj.rid, value, writeCallback);
    }
}
